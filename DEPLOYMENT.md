...
В build етапа се изпълнява `prisma db push` върху временна празна SQLite база
(`build-dummy.db`), защото `next build` рендерира статичните страници и има
нужда от съществуващи таблици. Тази база НЕ влиза в крайния образ; реалната
се създава от entrypoint-а в `/app/data`. Началната страница се рендерира при
всяка заявка (`force-dynamic`), за да не се „запече“ празна версия от билда.
